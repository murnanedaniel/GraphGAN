...
def load_dataset(datasplit, length):
    ...
    for data_num in datasplit:
        ...
        for i in tqdm(range(data_num)):
            ...
            nodes, edges = generate_segment(length)
            segments.append(Data(x=nodes, edge_index=edges, y=torch.tensor(length)))
...
def generate_segment(length = 1.):
    x0, y0 = np.random.rand(2)
    theta0 = np.random.rand()*np.pi
    
    seed_point = rotate(np.array([0, length/2]), theta0)
    O = np.array([x0, y0])
    nodes = np.tile(seed_point, (2, 1)).astype(float)
    rotations = np.linspace(0, 2*np.pi, 2+1)[:-1]
    
    for i, rotation in enumerate(rotations):
        nodes[i] = rotate(nodes[i], rotation)
        nodes[i] = nodes[i] + O
    
    FC_edges = torch.tensor([[0, 1]]).T
    
    return torch.from_numpy(nodes), FC_edges
